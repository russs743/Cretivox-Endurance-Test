...
export default function OutroSection() {
  ...
  useGSAP(() => {
    // 1. TRANSISI MASUK (WHITE REVEAL)
    // Lingkaran putih membesar dari tengah
    ...
    // 2. PINNING LOGIC (SOLUSI MASALAH KAMU)
    // Kita memaksa Kolom Kiri ("YOU CAN") untuk diam (Pinned)
    // selama Kolom Kanan ("Design", "Build") masih di-scroll.
    ScrollTrigger.create({
      trigger: containerRef.current, // Pemicunya adalah container utama
      start: "top top",              // Mulai pin saat bagian atas putih nyentuh atas layar
      end: "bottom bottom",          // Selesai pin saat bagian bawah putih nyentuh bawah layar
      pin: leftColRef.current,       // PIN KOLOM KIRI
      scrub: 1,
    });

    // 3. HIGHLIGHT TEXT (WARNA BERUBAH)
    const items = gsap.utils.toArray(".outro-item");
    items.forEach((item) => {
      gsap.fromTo(item, 
        { color: "#d1d5db", scale: 0.9, opacity: 0.3 }, 
        { 
          color: "#000000", // Ganti #ff0000 kalau mau Merah
          scale: 1.1, 
          opacity: 1,
          duration: 0.2,
          ease: "power1.out",
          scrollTrigger: {
            trigger: item,
            start: "top 55%",    // Nyala di tengah layar
            end: "bottom 45%",   
            scrub: true,
      ...
      <div className="w-full max-w-7xl mx-auto flex flex-col md:flex-row items-start relative">
        ...
        <div ref={leftColRef} className="w-full md:w-1/2 h-screen flex flex-col justify-center px-6 md:pl-12">
          <div>
            ...
            <p className="font-sans text-gray-400 text-lg md:text-xl mt-4 tracking-widest uppercase font-bold">
              MAKE IT REAL.
            </p>
          </div>
        </div>
